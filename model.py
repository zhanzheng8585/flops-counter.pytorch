...
class ColorizationNet(nn.Module):
    def __init__(self, midlevel_input_size=128, global_input_size=512):
        ...
        self.conv0 = nn.Conv2d(global_input_size, 256, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, global_output): #, global_input):
        
        # Convolutional layers and upsampling
        x = F.relu(self.bn1(self.conv0(global_output)))
...
class ColorNet(nn.Module):
    ...
    def forward(self, input_image):
        ...
        output = self.fusion_and_colorization_net(global_output) #, new_global_output)
        return output
